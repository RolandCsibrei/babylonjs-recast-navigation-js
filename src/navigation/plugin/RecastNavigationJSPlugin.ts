...
export class RecastNavigationJSPlugin implements INavigationEnginePlugin {
    ...
    private static _CreateNavMeshConfig<T extends INavMeshParameters>(
        parameters: T
    ): T extends { tileSize: number }
        ? Partial<TiledNavMeshGeneratorConfig>
        : SoloNavMeshGeneratorConfig {
        const cfg: any = {
            // The size of the non-navigable border around the heightfield. [Limit: >=0] [Units: vx] [0=]
        ...
        // If tileSize is present, return the config as TiledNavMeshGeneratorConfig
    ...
    createNavMesh(
        ...
        const cfg = RecastNavigationJSPlugin._CreateNavMeshConfig(parameters);
            ...
            const navMeshResult =
                "tileSize" in cfg
                    ? generateTiledNavMesh(
                          this._positions,
                          this._indices,
                          cfg as TiledNavMeshGeneratorConfig
                      )
                    : generateSoloNavMesh(this._positions, this._indices, cfg);

            if (!navMeshResult.success) {
                throw new Error(
                    `Unable to generateSoloNavMesh:${navMeshResult.error}`
                );
            }

            this.navMesh = navMeshResult.navMesh;

            this._navMeshQuery = new NavMeshQuery(navMeshResult.navMesh);
        }
    }
     ...
     */
    getClosestPoint(position: IVector3Like): Vector3 {
        const ret = this._navMeshQuery.findClosestPoint(position);
     ...
     */
    getClosestPointToRef(position: IVector3Like, result: Vector3): void {
        const ret = this._navMeshQuery.findClosestPoint(position);
     ...
     * @returns the closest point to position constrained by the navigation mesh
     */
    getRandomPointAround(position: IVector3Like, maxRadius: number): Vector3 {
    ...
    moveAlong(position: IVector3Like, destination: IVector3Like): Vector3 {
        const ret = this._navMeshQuery.moveAlongSurface(
            0,
            position,
            destination
        );
        const pr = new Vector3(
            ret.resultPosition.x,
            ret.resultPosition.y,
            ret.resultPosition.z
        );
        return pr;
    }
    ...
    computePath(start: IVector3Like, end: IVector3Like): Vector3[] {
        return this._convertNavPathPoints(
            this._navMeshQuery.computePath(start, end)
        );
    }
    ...
    computePathSmooth(
        ...
        return this._convertNavPathPoints(
            this._computeSmoothPath(this.navMesh, start, end, options)
        );
    }
    ...
    private _computeSmoothPath(
        ...
        const filter = options?.filter ?? this._navMeshQuery.defaultFilter;
        const halfExtents =
            options?.halfExtents ?? this._navMeshQuery.defaultQueryHalfExtents;
        ...
        const startNearestPolyResult = this._navMeshQuery.findNearestPoly(
            start,
            {
                filter,
                halfExtents,
        ...
        const endNearestPolyResult = this._navMeshQuery.findNearestPoly(end, {
            filter,
            halfExtents,
        });
        ...
        // find polygon path
        const findPathResult = this._navMeshQuery.findPath(
            startRef,
            endRef,
            start,
            end,
        ...
        const lastPoly = findPathResult.polys.get(
            findPathResult.polys.size - 1
        );

        let closestEnd = end;

        if (lastPoly !== endRef) {
            const lastPolyClosestPointResult =
                this._navMeshQuery.closestPointOnPoly(lastPoly, end);
        ...
        while (polys.length > 0 && smoothPath.length < maxSmoothPathPoints) {
            ...
            const steerTarget = RecastNavigationJSPlugin._GetSteerTarget(
                this._navMeshQuery,
                iterPos,
                targetPos,
                slop,
            ...
            const moveTarget = _moveTarget
                .copyFrom(iterPos)
                .addInPlace(delta.scale(len));

            // Move
            const moveAlongSurface = this._navMeshQuery.moveAlongSurface(
                polys[0],
                iterPos,
                moveTarget,
            ...
            RecastNavigationJSPlugin._FixupCorridor(
                polys,
                maxPathPolys,
                moveAlongSurface.visited
            );
            RecastNavigationJSPlugin._FixupShortcuts(polys, navMesh);

            const polyHeightResult = this._navMeshQuery.getPolyHeight(
                polys[0],
                result
            );
            ...
            if (
                isEndOfPath &&
                RecastNavigationJSPlugin._InRange(
                    iterPos,
                    steerTarget.steerPos,
                    slop,
                    1.0
                )
            ) {
                ...
                if (smoothPath.length < maxSmoothPathPoints) {
                    smoothPath.push(
                        new Vector3(iterPos.x, iterPos.y, iterPos.z)
                    );
                }
                ...
                const offMeshConnectionPolyEndPoints =
                    navMesh.getOffMeshConnectionPolyEndPoints(
                        prevPolyRef,
                        polyRef
                    );
                ...
                if (offMeshConnectionPolyEndPoints.success) {
                    if (smoothPath.length < maxSmoothPathPoints) {
                        ...
                        iterPos.copyFromFloats(
                            offMeshConnectionPolyEndPoints.end.x,
                            offMeshConnectionPolyEndPoints.end.y,
                            offMeshConnectionPolyEndPoints.end.z
                        );

                        const endPositionPolyHeight =
                            this._navMeshQuery.getPolyHeight(polys[0], iterPos);
    ...
    private static _GetSteerTarget(
        ...
        const straightPath = navMeshQuery.findStraightPath(
            start,
            end,
            pathPolys,
            {
    ...
    addCylinderObstacle(
        ...
        return (
            this._tileCache?.addCylinderObstacle(position, radius, height) ??
            (null as unknown as IObstacle)
        );
    }
    ...
    addBoxObstacle(
        ...
        return (
            this._tileCache?.addBoxObstacle(position, extent, angle) ??
            (null as unknown as IObstacle)
        );
    }
...
export class RecastJSCrowd implements ICrowd {
     ...
     */
    public constructor(
        plugin: RecastNavigationJSPlugin,
        maxAgents: number,
        maxAgentRadius: number,
        scene: Scene
    ) {
    ...
    overOffmeshConnection(index: number): boolean {
        return (
            this.recastCrowd.getAgent(index)?.overOffMeshConnection() ?? false
        );
    }
    ...
    updateAgentParameters(index: number, parameters: IAgentParameters): void {
        ...
        if (parameters.pathOptimizationRange !== undefined) {
            agentParams.pathOptimizationRange =
                parameters.pathOptimizationRange;
        }
